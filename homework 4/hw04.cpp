...
public:
    Warrior(const string& name, const int points) :
    ...
    void setStrength(int result) {strength = result;}
private:
    string warrior_name;
    int strength;
    bool is_hired;
...
public:
    ...
    int armyStrength() const {
        int total=0;
        for (Warrior* const& someone : army){
    ...
    // no return value
    void resultingStrength(int winner, int loser){
        for (Warrior* const& someone : army){
            int update = someone->getStrength() * ((winner - loser) / winner);
    ...
    void battle(Noble& enemy){
        ...
        if (is_alive && enemy.is_alive){
            int self_points = armyStrength();
            int enemy_points = enemy.armyStrength();
...
ostream& operator<<(ostream& os, const Noble& sir){
    ...
    for (const Warrior* const someone : sir.army){
